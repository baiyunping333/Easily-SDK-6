...
static void _plot_balancegram(const drawing_interface* pif, const plot_t* plt, matrix_t mt)
{
	...
	for (i = 0; i < x_count; i++)
	{
		...
		rl.fr = dx;
		rs.fr = 0;
		ft_calc_sector(&pt, &rl, &rs, fang, sang, pa, 4);
		...
		rl.fr = dy;
		rs.fr = 0;
		ft_calc_sector(&pt, &rl, &rs, fang, sang, pa, 2);
		t3.fx = pa[0].fx;
		t3.fy = pa[0].fy;
		...
		rl.fr = dy;
		rs.fr = 0;
		ft_calc_sector(&pk, &rl, &rs, fang, sang, pa, 4);
		k1.fx = pa[0].fx;
		k1.fy = pa[0].fy;
		k2.fx = pa[1].fx;
		k2.fy = pa[1].fy;
		...
		rl.fr = dy;
		rs.fr = 0;
		ft_calc_sector(&pk, &rl, &rs, fang, sang, pa, 2);
		k3.fx = pa[0].fx;
		k3.fy = pa[0].fy;
	...
	for (i = 0; i < 1; i++)
	{
		...
		for (j = 0; j < matrix_get_cols(mt); j++)
		{
			...
			if (pred != nxtd)
			{
				...
				rl.fr = rr - dd;
				rs.fr = rr - 2* dd;
				ft_calc_sector(&pt, &rl, &rs, (pred + nxtd) / 2, (pred + nxtd) / 2, pa, 4);
...
static void _plot_radargram(const drawing_interface* pif, const plot_t* plt, matrix_t mt)
{
	...
	ft_calc_equilater(&pt, &xn, pa, y_count);
...
static void _plot_fuelgram(const drawing_interface* pif, const plot_t* plt, matrix_t mt)
{
	...
	fang = XPI / 2;
	sang = 0;
	ft_calc_sector(&pt, &rl, &rs, fang, sang, pa, 1);
	...
	for (i = 0; i < 1; i++)
	{
		...
		for (j = 0; j < 7; j++)
		{
			...
			fang = 4 * XPI / 5 + j * dr;
			sang = dr;
			(*pif->pf_draw_sector)(pif->ctx, &xp, &xb_dot, &pt, &rl, &rs, fang, sang);
			...
			for (k = 0; k < 5; k++)
			{
				if (k < 4)
				{
					rl.fr = xr.fw / 2;
					rs.fr = rl.fr - dd / 2;
					ft_calc_sector(&pt, &rl, &rs, fang + k * (dr / 5), (dr / 5), pa, 4);
		...
		ft_calc_sector(&pt, &rl, &rs, fang, 0, pa, 1);
		...
		rs.fr = rl.fr - dd;
		sang = (fang - XPI / 20);
		ft_calc_sector(&pt, &rl, &rs, sang, 0, pa + 1, 1);
		...
		rs.fr = rl.fr - dd;
		sang = fang + XPI / 20;
		ft_calc_sector(&pt, &rl, &rs, sang, 0, pa + 3, 1);
