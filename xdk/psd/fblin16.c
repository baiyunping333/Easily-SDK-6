...
linear16_drawarea_bitmap_bytes_lsb_first(PSD psd, driver_gc_t * gc)
{
	...
	postfixbits = (gc->srcx + gc->width - 1) & 7;
	...
	if (first_byte != last_byte) {
		...
		size_main = 0;
		prefix_mask = 1 << prefixbits;
		prefix_last = 1 << (prefixbits + gc->width);
	}
	...
	if (gc->usebg) {
		...
		for (y = 0; y < gc->height; y++) {
			...
			for (t = 0; t < size_main; t++) {
				...
				if (0x01 & bitmap_byte) dst[0] = fg_color;
				if (0x02 & bitmap_byte) dst[1] = fg_color;
				if (0x04 & bitmap_byte) dst[2] = fg_color;
				if (0x08 & bitmap_byte) dst[3] = fg_color;
				if (0x10 & bitmap_byte) dst[4] = fg_color;
				if (0x20 & bitmap_byte) dst[5] = fg_color;
				if (0x40 & bitmap_byte) dst[6] = fg_color;
				if (0x80 & bitmap_byte) dst[7] = fg_color;
...
linear16_drawarea_alphamap(PSD psd, driver_gc_t * gc)
{
	...
	src = ((ADDR16)gc->data) + gc->srcx + gc->src_linelen * gc->srcy;
	...
	for (y = 0; y < gc->height; y++) {
		for (x = 0; x < gc->width; x++) {
			...
			*dst++ = ((as == 0) ? pd :
				((ad == 0) ? ps :
				low2scale[as | (ps >> 8)] + high2scale[as | (ps & 0xFF)] +
				low2scale[ad | (pd >> 8)] + high2scale[ad | (pd & 0xFF)]));
		}
		alpha += src_row_step;
		src += src_row_step;
		dst += dst_row_step;
...
{
	ADDR16	src = ((ADDR16)gc->data) + gc->srcx + gc->src_linelen * gc->srcy;
	ADDR16	dst = ((ADDR16)psd->addr) + gc->dstx + psd->linelen * gc->dsty;
	int	linesize = 2 * gc->width;
	int	y;

	DRAWON;
	for (y = 1; y < gc->height; y++) {
...
static void
linear16_drawarea(PSD psd, driver_gc_t * gc)
{
	assert(psd->addr != 0);
