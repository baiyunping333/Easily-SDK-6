...
void draw_equilagon_raw(visual_t rdc, const xpen_t* pxp, const xbrush_t* pxb, const xpoint_t* ppt, const xspan_t* pxn, int n)
{
	...
	pt_calc_equilater(ppt, pxn, pa, n);
...
void draw_equilagon(canvas_t canv, const xpen_t* pxp, const xbrush_t* pxb, const xpoint_t* ppt, const xspan_t* pxn, int n)
{
	...
	ft_calc_equilater(ppt, pxn, pa, n);
...
void draw_sector_raw(visual_t rdc, const xpen_t* pxp, const xbrush_t* pxb, const xpoint_t* ppt, const xspan_t* prl, const xspan_t* prs, double fang, double sang)
{
	if_context_t *pif;

	pif = PROCESS_CONTEXT_INTERFACE;

	(*pif->pf_gdi_draw_sector)(rdc, pxp, pxb, ppt, prl, prs, fang, sang);
}

void draw_sector(canvas_t canv, const xpen_t* pxp, const xbrush_t* pxb, const xpoint_t* ppt, const xspan_t* prl, const xspan_t* prs, double fang, double sang)
{
	visual_t rdc = get_canvas_visual(canv);
	xpoint_t pt;
	xspan_t rl, rs;
	...
	rl.r = tm_to_pt(canv, prl->fr, 0);
	rs.r = tm_to_pt(canv, prs->fr, 0);

	(*pif->pf_gdi_draw_sector)(rdc, pxp, pxb, &pt, &rl, &rs, fang, sang);
}
