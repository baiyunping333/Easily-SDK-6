...
int hand_textor_copy(textor_context* ptd)
{
	...
	if (!clipboard_put(ptd->widget, DEF_CB_FORMAT, (byte_t*)buf, (dword_t)(len * sizeof(tchar_t))))
	{
		xsfree(buf);
...
int hand_textor_paste(textor_context* ptd)
{
	...
	len = clipboard_get(ptd->widget, DEF_CB_FORMAT, NULL, MAX_LONG) / sizeof(tchar_t);
	if (!len)
	{
		return _TEXTOR_PRESS_IGNORE;
	}
	...
	buf = xsalloc(len + 1);
	clipboard_get(ptd->widget, DEF_CB_FORMAT, (byte_t*)buf, len * sizeof(tchar_t));
