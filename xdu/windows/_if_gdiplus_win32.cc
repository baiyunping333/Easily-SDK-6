...
void _gdiplus_draw_sector(visual_t rdc, const xpen_t* pxp, const xbrush_t* pxb, const xpoint_t* ppt, const xspan_t* prl, const xspan_t* prs, double from, double sweep)
{
	win32_context_t* ctx = (win32_context_t*)rdc;
	HDC hDC = (HDC)(ctx->context);
	...
	path.AddArc((ppt->x - prl->r), (ppt->y - prl->r), 2 * prl->r, 2 * prl->r, fdeg, tdeg);

	path.AddLine(pt[1].x, pt[1].y, pt[2].x, pt[2].y);

	path.AddArc((ppt->x - prs->r), (ppt->y - prs->r), 2 * prs->r, 2 * prs->r, fdeg + tdeg, -tdeg);

	path.AddLine(pt[3].x, pt[3].y, pt[0].x, pt[0].y);
...
void _gdiplus_draw_pie(visual_t rdc, const xpen_t* pxp, const xbrush_t*pxb, const xrect_t* prt, double fang, double sang)
{
	win32_context_t* ctx = (win32_context_t*)rdc;
	HDC hDC = (HDC)(ctx->context);
